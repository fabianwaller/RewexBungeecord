...
public class MuteManager {
	...
	public static void mute(String playername, String uuid, String mutedfrom, String reason, long seconds) {
	    ...
	    if (ProxyServer.getInstance().getPlayer(playername) != null) {
			ProxyServer.getInstance().getPlayer(playername).sendMessage(new TextComponent(Main.prefix + "§7Du wurdest von §9§lRewex.de§r §cgemuted§8!"));
			ProxyServer.getInstance().getPlayer(playername).sendMessage(new TextComponent(Main.prefix + "§7Grund: §c" + getReason(uuid)));
			ProxyServer.getInstance().getPlayer(playername).sendMessage(new TextComponent(Main.prefix + "§7Verbleibende Zeit: §c" + getRemainingTime(uuid)));
			ProxyServer.getInstance().getPlayer(playername).sendMessage(new TextComponent(Main.prefix + "§7Gemuted von: " + getMutedFrom(uuid)));
	    }
	}
